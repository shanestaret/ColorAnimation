...
/**
 * Class that allows Java to manipulate the Arduino, displays the GUI, and writes a text file for the Arduino to understand
 */

public class RXTX implements SerialPortEventListener {//RXTX class implementing serialporteventlistener
    ...
    public static void main(String[] args) throws Exception {//if anything goes wrong it will throw exception
        ...
        RXTX main = new RXTX();//main object created
        ...
        for (int i = 0; i < numOfTimesToLoop; i++) { //loop as many times as there are subfolders
            ...
            tp = new Tabs(newPath, path); //creates the tabs to hold both figures
            tp.createTabs(); //creates the tabs to hold both figures

            maxColorIndex = ColorProportions.retrieveMaxColor(); //uses the "retrieveMaxColor" method in the "ColorProportions" class to get the index that has the greatest proportion
         ...
         */
            FilePicker.create(maxColorIndex, path); //creates a file that contains the index that has the largest color proportion

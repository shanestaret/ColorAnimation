...
newFolderNameArray = strsplit(fileDesired,'.'); %separating the name of the image/video from its extension
if(isAVid) %so we can specify in the folder name whether we are looking at a video or an image
...
subPlotManip = 2; %if we're only looking at one image, put the manipulated image in the second subplot
...
if(isAVid) %if the file is a video
    subPlotManip = 3; %manipulated image looks better in third subplot during a video
    ...
    subPlotThreeD = [10; 12]; %need more plot space for video, so these get pushed back to plots 10 and 11
...
if(firstFrame)
nameForFrame = 1; %name that we will name the final frame to put into the folder, so if we are looking at our third frame, then nameForFrame = 3
bars = figure('units','normalized','outerposition',[0.488 0 0.517 1], 'Name', 'Bar Graphs') %creates a figure that holds the bar graphs on the right
main = figure('units','normalized','outerposition',[-0.004 0 0.5 1], 'Name', 'Color Variation'); %creates a figure that holds everything but the bar graphs on the left
...
subplot(4,subPlotCol,1), imshow(InitialImage); title(['Original Image (Frame ' num2str(frameCount) '/' num2str(totalFrames) ')'],'FontSize', 16); %displays original image
subplot(4,subPlotCol,subPlotManip), imshow(ManipulatedImageFinal, []); title(['Manipulated Image (Frame ' num2str(frameCount) '/' num2str(totalFrames) ')'],'FontSize', 16); %displays manipulated image
...
mainDuplicate = copyobj(main, 0);
set(mainDuplicate,'units','normalized','outerposition',[0 0 1 1],'Visible', 'Off');
...
imwrite([X, Map], strcat(fullfile(folder, newFolderName),'\Frame', int2str(nameForFrame), 'Main.png')); %writes the image of the figure to an image file
...
drawnow
barsDuplicate = copyobj(bars, 0);
set(barsDuplicate,'units','normalized','outerposition',[0 0 1 1],'Visible', 'Off');
barFigure = getframe(barsDuplicate); %creates object that holds the figure
[X, Map] = frame2im(barFigure); %creates a matrix that converts the figure into an image
imwrite([X, Map], strcat(fullfile(folder, newFolderName),'\Frame', int2str(nameForFrame), 'Bars.png')); %writes the image of the figure to an image file
if(isAVid)
...
nameForFrame = nameForFrame + 1; %signaling that we will be looking at the next frame
end
end
fileInfoCat = strcat(newFolderName, '>', int2str(numberOfFramesDisplayed)); %String of info that we need to send to Java in order for it to read the images processed in MATLAB
fileInfo = fopen(fullfile(fullfile(folder, newFolderName),'fileInfo.txt'), 'w' ); %creates a text file which we will write the folder name to along with how many frames we are looking at
fprintf(fileInfo, '%s', fileInfoCat); %writes the original image proportions to this text file so we can use the proportions in Java
fclose(fileInfo); %closes the file
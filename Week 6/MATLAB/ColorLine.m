classdef ColorLine %class to create color variation line
    ...
    methods
        ...
        function drawLine = drawLine(lineObj)
            bound = lineObj.ColorProps(1); %the y-value that we start to draw the rectangle at; equivalent to the upper y-value of last rectangle drawn; so if there are three rectangles on top of one another and the second one ends at y = 15, then bound = 15, and bound will be used to determine the lower y-value for the next rectangle 
            ...
            for i=1:length(lineObj.ColorProps)
                if i == 1
            linePortion = rectangle('Position', [lineObj.ColorLineXStart 0 1 bound], 'FaceColor', lineObj.ColorValues(i,:),'LineStyle','none'); %draws the first line, since we are looking at the propOfWhite, we make the line white; we change the width as well to make it more visible
                else
            linePortion = rectangle('Position', [lineObj.ColorLineXStart bound 1 lineObj.ColorProps(i)], 'FaceColor', lineObj.ColorValues(i,:),'LineStyle','none'); %draws the first line, since we are looking at the propOfWhite, we make the line white; we change the width as well to make it more visible
                end
            if i > 1
            bound = bound + lineObj.ColorProps(i);
            end
            end

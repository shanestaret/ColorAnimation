classdef ThreeDCluster < GraphInterface %implements the interface "GraphInterface"; creates a 3D plot
	    ...
	    methods
	        function threeDObj = ThreeDCluster(d,t)% variables being passed into the parameter 
              ...
              %the threeDObj
                ...
                threeDObj.propOfWhite = d(1); %assigns the propOfWhite to variable w
                threeDObj.propOfBlack = d(2); %assigns the propOfBlack to variable z
                threeDObj.propOfGrey = d(3); %assigns the propOfGrey to variable y
                threeDObj.propOfRed = d(4); %assigns the propOfRed to variable r
                threeDObj.propOfGreen = d(5); %assigns the propOfGreen to variable g
                threeDObj.propOfBlue = d(6); %assigns the propOfBlue to variable b
                threeDObj.xLabel = 'x';
                threeDObj.yLabel = 'y';
                threeDObj.zLabel = 'z';
            end %Terminates block of code
	        ...
	        function createGraph(threeDObj) % function created to create a graph
                ...
                plot3(sixthClust(:,1),sixthClust(:,2),sixthClust(:,3), 'blue.','LineWidth', 3,'MarkerEdgeColor', 'blue', 'MarkerSize', 12); hold off
                grid on %turns the grid on, makes 3D plot more visible
                set(gca, 'Color', [0.3 0.9 1]); title(threeDObj.Title, 'FontSize', 16); axis([-40 40 -50 50 -40 40]); xlabel(threeDObj.xLabel); ylabel(threeDObj.yLabel); zlabel(threeDObj.zLabel) %sets background color of the plot to cyan so that it is easier to see the dots

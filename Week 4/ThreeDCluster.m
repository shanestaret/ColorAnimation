classdef ThreeDCluster < GraphInterface %implements the interface "GraphInterface"; creates a 3D plot
	    ...
	    methods
	        ...
	        function createGraph(threeDObj) % function created to create a graph
                ...
                set(gca, 'Color', [0.3 0.9 1]); title(threeDObj.Title, 'FontSize', 16); xlabel(threeDObj.xLabel); ylabel(threeDObj.yLabel); zlabel(threeDObj.zLabel); axis([-250 125 -250 250 -250 250]); %sets background color of the plot to cyan so that it is easier to see the dots, while also labelling the axes, creating a title, and setting the limits of the axes
            end
        end
end
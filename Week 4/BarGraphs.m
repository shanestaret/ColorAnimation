classdef BarGraphs < GraphInterface %implements the interface "GraphInterface"; to create bar graphs
    ...
    methods
        ...
        function createGraph(graphObj)
            ...
            graphObj.Plot = bar(graphObj.Data, graphObj.BarWidth, 'FaceColor',[0 .5 .5],'EdgeColor',[0.4 0.4 0.4],'LineWidth',1.5'); title(graphObj.Title, 'FontSize', 16); ylim([0 100]); xlim([0 col1 + 1]); %plotting of the actual graph; it plots the data, giving each bar a color, creating an outline for each bar, and giving each axis a title, along with a title to the whole bar graph
            % --------------------------------
